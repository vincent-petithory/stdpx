						...
						<h3>
							<xsl:choose>
								<xsl:when test="$isTopLevel='true'">
									<a href="package-detail.html" target="classFrame">Top Level</a>
								</xsl:when>
								<xsl:otherwise><a href="package-detail.html" target="classFrame">Package <xsl:value-of select="@name"/></a></xsl:otherwise>
							</xsl:choose>
						</h3>
						<table cellpadding="0" cellspacing="0">
							<xsl:for-each select="fields/field[@isConst='true']">
								...
								<xsl:if test="position()=1">
									<tr>
										<td>
											<a href="package.html#constantSummary"><b><xsl:text>Constants</xsl:text></b></a>
										</td>
									</tr>
								</xsl:if>
								<tr>
							...
							<xsl:for-each select="fields/field[@isConst='false']">
								...
								<xsl:if test="position()=1">
									<tr>
										<td>
											<a href="package.html#propertySummary"><b><xsl:text>Properties</xsl:text></b></a>
										</td>
									</tr>
								</xsl:if>
								<tr>
							...
							<xsl:for-each select="methods/method">
								...
								<xsl:if test="position()=1">
									<tr>
										<td>
											<a href="package.html#methodSummary"><b><xsl:text>Functions</xsl:text></b></a>
										</td>
									</tr>
								</xsl:if>
								<tr>
